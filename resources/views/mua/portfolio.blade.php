...
<div class="space-y-5">
    ...
    <div class="bg-white border border-slate-200/60 rounded-2xl shadow-xs p-5 space-y-3">
        <h3 class="text-xs font-bold text-slate-700">Unggah Karya Baru</h3>
        <form action="{{ route('mua.portfolio.store') }}" method="POST" enctype="multipart/form-data" class="space-y-3">
            @csrf
            <div class="space-y-1.5">
                <label class="block text-[11px] font-semibold text-slate-600">Pilih Foto / Video</label>
                <input type="file" name="files[]" multiple accept="image/jpeg,image/png,image/webp,video/mp4,video/quicktime" required
            ...
            <div class="space-y-1.5">
                <label class="block text-[11px] font-semibold text-slate-600">Caption (opsional)</label>
                ...
                <p class="text-[10px] text-slate-400">Caption yang sama akan diterapkan ke semua file yang diunggah pada sesi ini.</p>
            </div>
            <button type="submit"
    ...
    <div class="space-y-3">
        ...
        @if($portfolios->isEmpty())
            <div class="bg-white border border-slate-200/60 rounded-2xl shadow-xs p-8 text-center text-xs text-slate-400">
                Belum ada karya portofolio. Unggah karya pertama Anda di atas.
            </div>
        @else
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                @foreach($portfolios as $item)
                    <div class="bg-white border border-slate-200/60 rounded-2xl shadow-xs overflow-hidden group relative">
                        ...
                        <div class="p-3 space-y-2">
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <span class="px-1.5 py-0.5 rounded-full bg-slate-100 text-slate-400 text-[8px] font-bold uppercase flex-shrink-0">{{ $item->file_type }}</span>
                            </div>
